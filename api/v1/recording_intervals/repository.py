...
class RecordingIntervalRepository:
    ...
    def get_by_id(
        self, recording_interval_id: int, recording_id: int
    ) -> RecordingInterval | None:
        return (
            self.db.query(RecordingInterval)
            .filter(
                RecordingInterval.id == recording_interval_id,
    ...
    def get_all(
        self, recording_id: int, skip: int = 0, limit: int = 100
    ...
    def batch_create(
        self, recording_intervals: list[RecordingInterval]
    ) -> list[RecordingInterval]:
    ...
    def batch_delete_with_recording_id(self, recording_id: int) -> None:
        self.db.query(RecordingInterval).filter(
            RecordingInterval.recording_id == recording_id
        ).delete()
        self.db.commit()
