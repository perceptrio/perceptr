...
def create_recording_interval(
    db: Session, recording_interval: RecordingIntervalCreate
) -> RecordingInterval:
    """Create a new recording interval"""

    # Create recording
    repository = RecordingIntervalRepository(db)
    recording_interval = repository.create(
        RecordingInterval(
...
def get_recording_interval(
    db: Session, recording_interval_id: int, recording_id: int
) -> RecordingInterval:
    repository = RecordingIntervalRepository(db)
...
def get_recordings(
    db: Session, recording_id: int, skip: int = 0, limit: int = 100
) -> list[RecordingInterval]:
    repository = RecordingIntervalRepository(db)
    return repository.get_all(recording_id, skip, limit)


def get_recording_intervals_by_recording_id(
    db: Session, recording_id: int
) -> list[RecordingInterval]:
    repository = RecordingIntervalRepository(db)
    return repository.get_by_recording_id(recording_id)


def update_recording_interval(
    db: Session, recording_interval_id: int, recording_interval: RecordingIntervalUpdate
) -> RecordingInterval:
    repository = RecordingIntervalRepository(db)
    return repository.update(recording_interval)


def soft_delete_recording_interval(
    db: Session, recording_interval_id: int, recording_id: int
) -> None:
    repository = RecordingIntervalRepository(db)
    recording_interval = repository.get_by_id(recording_interval_id, recording_id)
...
def batch_create_recording_intervals(
    db: Session, recording_intervals: list[RecordingInterval]
) -> list[RecordingInterval]:
    repository = RecordingIntervalRepository(db)
    return repository.batch_create(recording_intervals)


def replace_recording_intervals(
    db: Session, recording_id: int, recording_intervals: list[RecordingInterval]
) -> list[RecordingInterval]:
    repository = RecordingIntervalRepository(db)
