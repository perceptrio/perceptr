...
import cv2
import numpy as np
from common.services.logger import logger  # Import the logger


def preprocess_recording(
    recording_path: str, frames_per_second: int = 1
) -> List[Tuple[int, List[Tuple[float, np.ndarray]]]]:
    """
...
def timestamp_frames(
    frames: List[Tuple[float, np.ndarray]], start_time: int, frames_per_second: int
) -> List[Tuple[str, np.ndarray]]:
    """
...
def create_video_from_frames(
    frames: List[np.ndarray], output_path: str, fps: int = 1
) -> str:
    """
...
def slow_down_video(
    frames: List[np.ndarray], slowdown_factor: float = 2.0
) -> List[np.ndarray]:
    """
...
def enhance_frame(
    frame: np.ndarray, brightness: float = 1.0, contrast: float = 1.0
) -> np.ndarray:
    """
...
def extract_frames_at_timestamps(
    recording_path: str, timestamps: List[float]
    ...
    Args:
        recording_path (str): Path to the video file
        timestamps (List[float]): List of timestamps in seconds to extract frames at

    Returns:
        List[Tuple[float, np.ndarray]]: List of tuples containing:
            - Precise timestamp in seconds (float)
            - Frame data as numpy array
    ...
    # Return empty list if no timestamps provided
    if not timestamps:
        return []
    ...
    try:
        ...
        # Sort timestamps for more efficient frame extraction
        # (minimizes back-and-forth seeking)
        ...
        for timestamp in sorted_timestamps:
            ...
            # Try two different seeking methods for better accuracy
            ...
            # If first method fails, try seeking by milliseconds
            if not ret:
                logger.info(
                    f"Seek by frame# failed for {timestamp:.3f}s in {recording_path}. Trying seek by ms."
                )
                cap.set(cv2.CAP_PROP_POS_MSEC, timestamp * 1000)
                ret, frame = cap.read()

            if ret:
                # Keep the original float timestamp for precision
                frames.append((timestamp, frame))
            else:
                # Log a warning if frame extraction failed for this timestamp
                logger.warning(
                    f"Failed to extract frame for timestamp {timestamp:.3f}s in video {recording_path}"
                )
