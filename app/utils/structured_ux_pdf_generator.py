...
from graphs.ux_audit_graph import Issue, UXAudit
...
class StructuredUXAuditPDFGenerator:
    ...
    def _resize_image_if_needed(
        ...
        try:
            ...
            logger.warning(
                f"Could not calculate image dimensions for {image_path}: {e}"
            )
    ...
    def generate_pdf(
        self,
        frames_data: List[
            Tuple[str, str, UXAudit]
        ],  # (frame_path, timestamp, ux_audit)
        output_path: str,
        ...
        try:
            ...
            story.append(
                Paragraph(f"Generated on: {timestamp}", self.styles["Metadata"])
            )
            ...
            story.append(
                Paragraph(
                    f"Total screens analyzed: {len(frames_data)}",
            ...
            for idx, (frame_path, timestamp, ux_audit) in enumerate(frames_data, 1):
                ...
                story.append(
                    Paragraph(f"Screen {idx} Analysis", self.styles["FrameTitle"])
                )
                story.append(Spacer(1, 10))

                # Add frame timestamp
                story.append(
                    Paragraph(f"Timestamp: {timestamp}", self.styles["Metadata"])
                )
                ...
                if ux_audit.short_title:
                    story.append(
                        Paragraph(
                            f"Screen: {ux_audit.short_title}",
                            self.styles["ScreenTitle"],
                        )
                    )
                    story.append(Spacer(1, 10))
                ...
                if os.path.exists(frame_path):
                    try:
                        ...
                        story.append(
                            Paragraph(
                                f"[Image not available: {os.path.basename(frame_path)}]",
                                self.styles["Metadata"],
                            )
                        )
                        story.append(Spacer(1, 15))
                    ...
                    story.append(
                        Paragraph(
                            f"[Image not found: {os.path.basename(frame_path)}]",
                            self.styles["Metadata"],
                        )
                    )
                    story.append(Spacer(1, 15))
                ...
                # Add issues section
                story.append(
                    Paragraph("Issues & Recommendations", self.styles["SectionHeading"])
                )
                ...
                if ux_audit.issues:
                    ...
                    for issue_idx, issue in enumerate(ux_audit.issues, 1):
                        story.append(
                            Paragraph(
                                f"{issue_idx}. {issue.issue_title}",
                                self.styles["IssueTitle"],
                            )
                        )
                        story.append(
                            Paragraph(
                                f"<b>Issue:</b> {issue.issue}",
                    ...
                    story.append(
                        Paragraph(
                            "✓ No issues identified for this screen.",
                            self.styles["NoIssues"],
                        )
                    )
                    story.append(Spacer(1, 15))
            ...
            logger.info(
                f"Structured UX audit PDF report generated successfully: {output_path}"
            )
            return output_path
