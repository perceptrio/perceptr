...
class RecordingSummary(BaseModel):
    summary: str = Field(description="A summary of the user's actions, behavior, emotional state. Also include any issues found in the recording and recommendations for improvement. The summary should be in the form of a story of the user's experience, from beginning to end.")
    short_title: str = Field(description="A short title for the recording. It should be a single sentence that captures the main idea of the recording.")
...
class RecordingSummarizerGraph():
    ...
    def recording_summarizer(self, state: State, config: RunnableConfig) -> dict:
        ...
        response = self.openai_llm.with_structured_output(RecordingSummary).invoke(messages)
       
        return {
            "recording_summary": response
        }
    ...
    @observe()
    def summarize_recording(self, org_id: str, recording_id: str, recording_intervals_summary: str) -> dict:
