...
class NewIssue(BaseModel):
    ...
    issue_recommendation: str = Field(description="A recommendation on how to fix/enhance the issue.")
    issue_severity: str = Field(description="The severity of the issue. It should be one of the following: LOW, MEDIUM, HIGH")
    issue_category: str = Field(description="The category of the issue. It should be one of the following: BUG, USABILITY_ISSUE, PERFORMANCE_ISSUE, ENHANCEMENT")

class ExistingIssue(BaseModel):
    issue_id: int = Field(description="The id of the issue.")

class Issue(BaseModel):
    recording_interval_id: int = Field(description="The id of the recording interval that the issue belongs to.")
    is_new_issue: bool = Field(description="Whether the issue is new or not. If it is a new issue, it should be True, otherwise it should be False.")
    issue: Union[NewIssue, ExistingIssue] = Field(description="The issue found in the recording interval. If it is a new issue, it should be a NewIssue object, otherwise it should be an ExistingIssue object.")
...
class IssuesSummarizerGraph():
    ...
    @observe()
    def aggregate_issues(self, org_id: str, recording_id: str, analyzed_recording_issues: list[dict], existing_issues: list[dict]) -> dict:
