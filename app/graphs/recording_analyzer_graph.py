...
class TimestampDescription(BaseModel):
    """A timestamp in the recording. Use the timestamp to describe the user's actions."""
...
class RecordingIntervals(BaseModel):
...
class State(TypedDict):
    ...
    recording_intervals: RecordingIntervals
...
class RecordingAnalyzerGraph():
    def __init__(self):
        ...
        graph_builder.add_node("extract_timestamped_intervals", self.extract_timestamped_intervals)

        graph_builder.add_edge(START, "recording_analyzer")
        graph_builder.add_edge("recording_analyzer", "extract_timestamped_intervals")
        graph_builder.add_edge("extract_timestamped_intervals", END)
    ...
    def recording_analyzer(self, state: State, config: RunnableConfig) -> dict:
        ...
        llm = configuration.get("llm", "GEMINI")
