    ...
    try:
        ...
        cmd = [
            ...
            *hw_accel,
            ...
            video_path,
            "-filter:v",
            f"setpts={slowdown_factor}*PTS",  # Slow down video
            *audio_filter,
        ]
...
def ffmpeg_chunk_video(
    ...
    codec: Optional[
        str
    ] = None,  # Keeping parameter for backward compatibility, but we won't use it
    ...
    try:
        ...
        if chunk_count <= 3:
            return ffmpeg_chunk_video_optimized(
                video_path,
                chunk_size_seconds,
                output_dir,
                None,
        ...
        for i in range(chunk_count):
            ...
            chunk_path = os.path.join(
                output_dir, f"{base_filename}_chunk_{i}{file_ext}"
            )
            ...
            print(
                f"Creating chunk {i+1}/{chunk_count} from {start_seconds}s to {start_seconds + duration}s"
            )
...
def slow_down_video(
    video_path: str, output_path: str, slowdown_factor: float = 2.0
) -> None:
    """
    ...
    try:
        ...
        if not out.isOpened():
            ...
            print(
                f"Failed to open video writer with codec {codec_str}, falling back to mp4v"
            )
            ...
            out = cv2.VideoWriter(
                fallback_path, fallback_fourcc, new_fps, (width, height)
            )
...
def chunk_video(
    ...
    try:
        ...
        chunk_count = max(
            1,
            int(total_duration / chunk_size_seconds)
            + (1 if total_duration % chunk_size_seconds > 0 else 0),
        )
        ...
        for i in range(chunk_count):
            ...
            if not out.isOpened():
                ...
                print(
                    f"Failed to open video writer with codec {codec_str}, trying fallback"
                )
                fallback_ext = ".mp4"
                fallback_path = os.path.join(
                    output_dir, f"{base_filename}_chunk_{i}{fallback_ext}"
                )
                fallback_fourcc = cv2.VideoWriter_fourcc(*"mp4v")
                out = cv2.VideoWriter(
                    fallback_path, fallback_fourcc, fps, (width, height)
                )
            ...
            print(
                f"Created chunk {i+1}/{chunk_count} from {start_seconds}s to {start_seconds + duration}s"
            )
...
def extract_frames_from_video(
    ...
    try:
        ...
        frame_interval = (
            video_fps / fps
        )  # How many video frames to skip between extractions
        ...
        while True:
            ...
            if current_frame >= frame_count * frame_interval:
                ...
                if not success:
                    logger.warning(
                        "Failed to save frame for video while extracting frames",
        ...
        logger.info(
            "Successfully extracted frames from video",
